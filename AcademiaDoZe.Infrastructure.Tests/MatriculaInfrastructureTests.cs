...
public class MatriculaInfrastructureTests : TestBase
{
    ...
    Arquivo? laudo = null)
    {
        ...
        var obsFinal = string.IsNullOrEmpty(obsRestricao) ? "SQLite" : $"{obsRestricao} - SQLite";
        var matriculaResult = Matricula.Criar(
        id: 0, aluno: aluno, plano: plano, dataInicio: inicio, objetivo: "Matheus", restricoesMedicas: restricoes, laudoMedico: laudo, observacoesRestricoes: obsFinal
        );
    ...
    public async Task Matricula_Adicionar_E_ObterPorId_Sucesso()
    {
        ...
        Assert.Equal("Usar medicação ao acordar - SQLite", obtida.ObservacoesRestricoes);
    ...
    public async Task Matricula_RestricoesMedicas_ComVariacoesMultiplaEscolha_PersisteEObtemCorretamente()
    {
        ...
        Assert.Equal("Evitar exercícios de alto impacto e hipertensão - SQLite", obtida.ObservacoesRestricoes);
        Assert.NotNull(obtida.LaudoMedico);
        Assert.Equal(laudo.Conteudo, obtida.LaudoMedico.Conteudo);
    }
    [Fact]
    ...
    public async Task Matricula_Atualizar_Sucesso()
    {
        ...
        Assert.Equal("Matheus", noBanco.Objetivo);
